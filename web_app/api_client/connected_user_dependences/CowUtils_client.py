...
class CowUtilsClient:
    ...
    def update_cow(self, cow_id: int, **kwargs: dict[str, Any]) -> None:
        ...
        if "cow_cares" in kwargs:
            raise NotImplementedError("La modification de cow_cares doit être gérée via les fonctions d'ajout, de suppression et de modification de traitement dédiées pour assurer la cohérence du stock de médicaments.")
    ...
    def remove_cow(self, cow_id: int) -> None:
        ...
        """
        cow: Cow_Client | None
        if cow := self.get_cow(cow_id):
            if not cow.in_farm:
                raise ValueError(
                    f"user :{self}, cow: {cow_id}: deja supprimé.")
            if CowUtils.remove_cow(user_id=self.connected_user.id, cow_id=cow_id) or True: #TODO remplaser au passage a l'api
                 cow.in_farm = False
                 lg.info(f"(user :{self}, cow: {cow_id}): left the farm.")
            else :
                lg.error(f"(user: {self}, cow: {cow_id}, fonction: remove_cow): remove_cow failed")
                raise ValueError(
                    f"(user :{self}, cow: {cow_id}) : remove_cow failed")
        else:
    ...
    def add_calf(self, calf_id: int,
        ...
        if not self.get_cow(calf_id):
            if CowUtils.add_cow(
                ...
                lg.warning(f"(user :{self}, cow: {calf_id}) : upload failed")
        else:
            lg.error(
    ...
    def add_cow_care(
        ...
        cow: Cow_Client | None
        if cow := self.get_cow(cow_id):
            if CowUtils.add_care(cow, cow_care):
                return self.add_care(cow, cow_care)
            else:
                lg.error(f"(user: {self}, cow: {cow_id}, fonction: add_cow_care): add_care failed")
        lg.error(f"(user :{self}, cow: {cow_id})  not found.")
        raise ValueError(f"(user :{self}, cow: {cow_id})  n'existe pas.")

    
    def add_care(self,
        cow: Cow_Client, cow_care: Traitement
    ) -> tuple[int, date | None]:
