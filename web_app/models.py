...
def update_care(
    ...
    if cow := Cow.query.get(id):
        return add_care(cow, cow_care, id)
...
def get_all_care() -> (
    ...
    all_cares: List[Tuple[date, dict[str, int]]] = [
        care for cow in Cow.query.all() for care in cow.cow_cares
    ]
    all_cares.sort(
        key=lambda care: care[0], reverse=True
